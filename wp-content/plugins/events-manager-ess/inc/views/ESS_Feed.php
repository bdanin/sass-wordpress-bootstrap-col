	...
	public static function output( $event_id=NULL, $page=1, $is_download=FALSE, $is_push=FALSE )
	{
		...
		while ( count( $EM_Events ) > 0 )
		{
			foreach ( $EM_Events as $ie => $EM_Event )
			{
				...
				//dd( wpautop( $EM_Event->post_content, TRUE ) );
				...
				if ( isset( $EM_Event->event_start_date ) && isset( $EM_Event->event_end_date ) )
				{
					...
					{
						$duration_h = FeedValidator::getDateDiff( 'h', $event_start, $event_stop ); // number of hours between two dates

						$newEvent->addDate( 'standalone', 'hour', NULL,NULL,NULL,NULL, array(
							'name'		=> sprintf( __( 'Date: %s', 'dbem'), $event_start ),
							'start'		=> $event_start,
							'duration'	=> ( ( $duration_h > 0 )? $duration_h : 0 )
						) );
					}
					// -- RECURCIVE -----
					else
							...
							if ( isset( $prices_->tickets ) )
							{
								if ( is_array( $prices_->tickets->tickets ) )
								{
									foreach ( $prices_->tickets->tickets as $i => $price )
									{
										if ( is_object( $price ) )
										{
											$duration_h = ( ( $price->ticket_start && $price->ticket_end )? FeedValidator::getDateDiff( 'h', $price->ticket_start, $price->ticket_end ) : 0 );
											...
											$newEvent->addPrice( 'standalone', $t_mode, "hour",NULL,NULL,NULL,NULL, array(
												...
												'duration' 	=> ( ( intval( $duration_h ) > 0 )? $duration_h : 0 ),
